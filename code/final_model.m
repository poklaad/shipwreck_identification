...
% Параметр, использующийся при моделировании нерегулярного волнения
cosn = 50;

% Если не хватило файлов с волнением, будет использоваться моделируемое
% нерегулярное волнение. Булева переменная unreg_waves_using отвечает за
% проверку, нужно ли использовать нерегулярное волнение
% least_count - наименьшее из всех чисел примеров для каждого типа затопления
unreg_waves_using = false;
least_count = 0;

while least_count < 100
    if unreg_waves_using
        pose2_arr = [2 3 4 5 6];
        pose2_arr = pose2_arr(less100);
    end
    for pose2_i = 1:length(pose2_arr)
        ...
        for file_i = 1:length(wave_file_list)
            ...
            % A_real,A_unreg  - амплитуда волнения для реальное и
            % моделируемого волнения соотвественно. Амплитуды разные из-за
            % высоких значений выбросов в реальном волнении.
            ...
            % используется 4000. Первый шаг 0-4000, второй 2000-6000, третий
            % 4000-8000, четвертый 6000-10000
            for waving_start = 1:2000:6000
                tic
                ...
                if unreg_waves_using
                    wv = waves(start_time, alltime, 1, cosn);
                    
                    % Масштабирование волнения. A_unreg - максимальная по модулю волна
                    [tmp, ind1] = max(abs(wv));
                    scale_wave = A_unreg / tmp;
                    wv = scale_wave * wv;
                else
                ...
                if unreg_waves_using
                    output = ['Modeling with ', num2str(pose2 - 1), ' type of wreck', num2str(file_i), ' waving_file cycle out of 8, ', num2str(waving_start), ' start of waving cycle out of [1, 2001, 4001, 6001] but with unregular waving is finished'];
                else
                    output = ['Modeling with ', num2str(pose2 - 1), ' type of wreck, ', num2str(file_i), ' waving_file, ', num2str(waving_start), ' out of [1, 2001, 4001, 6001] start of waving is finished'];
                end
                disp(output)
                toc
    ...
    % Вычисление сколько примеров каждого типа было сгенерировано. Если
    % есть типы затопления с примерами меньше 100, моделирование начинается
    % заново, но только для тех типов, где примеров меньше 100
    ...
    less100 = find(count < 100);
    least_count = min(count);
    if ~isempty(less100)
        unreg_waves_using = true;
    end
...
file_norm_autocor = fopen('norm_autocor.txt','a');
file_norm_wreck_type = fopen('norm_wreck_type.txt','a');
